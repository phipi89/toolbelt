...
class SpawnTests(unittest.TestCase):
    def test_target_context_uses_focused_window_from_snapshot(self) -> None:
        focused = {"display": 2, "space": 7, "has-focus": True}

        with (
            patch.object(spawn.yabai, "query_window") as query_window,
            patch.object(spawn.yabai, "query_display") as query_display,
        ):
            self.assertEqual(spawn._target_display_and_space([focused]), (2, 7))
    ...
    def test_target_fallback_uses_display_index(self) -> None:
        ...
        with (
            patch.object(spawn.yabai, "query_window", side_effect=error),
            patch.object(
                spawn.yabai,
                "query_display",
