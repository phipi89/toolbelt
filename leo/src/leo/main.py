...
class LeoRequestManager:
    ...
    def get_xml(self, url, verbose=False):
        ...
        if self.recentlySent:
            if verbose:
                print("Pausing between requests to LEO server...", file=sys.stderr)
            time.sleep(1)
...
def get_entries(word, source_lang, target_lang, pos_filter, verbose=False):
    xml = _retrieve_translation_doc(word, source_lang, target_lang, verbose=verbose)
        ...
        for entry in entries:
            ...
            for side in entry.find_all("side"):
                ...
                if flecttab:
                    ...
                    inflect_url = None
                parsed_sides.append(
                    {"word": word, "inflect_url": inflect_url, "lang": lang}
                )
...
def _retrieve_translation_doc(word, source_lang, target_lang, verbose=False):
    url = TRANSLATE_URL % {"lang": LANG_LOOKUP[source_lang + target_lang], "word": word}
...
def _format_noun_case(case):
    ...
    return {
        "name": case_name,
        "value": (art.get_text() + " " if art else "") + radical + ending,
    }
...
def _get_tables(entries, verbose=False):
    ...
    for entry in entries:
        target_side = next(
            (side for side in entry["sides"] if side["lang"] == TARGET_LANG)
        )
...
def _print_translation(translation, args):
    word_table = []
    for entry in translation[0 : args["max_entries"]][::-1]:
        word_table.append([side["word"] for side in entry["sides"]])
    print(
        tabulate(
            word_table,
            headers=[
                LANGUAGES.get(args["source_lang"], args["source_lang"]),
                LANGUAGES.get(args["target_lang"], args["target_lang"]),
            ],
        )
    )
...
def build_parser():
    ...
    p.add_argument(
        ...
        help="target language, 2 chars (e.g. 'en')",
    )
    p.add_argument(
        "-t",
        "--to",
        ...
        default="de",
        choices=COUNTRY_CODES,
        help="source language, 2 chars (e.g. 'de')",
    )
    p.add_argument(
        "-m",
        "--max-entries",
        ...
        default=999,
        type=int,
        help="maximal number of entries to print",
    )
    p.add_argument(
        "-i",
        "--inflect",
...
def parse_args(argv=None):
    ...
    if not args["inflect"]:
        args["define"] = True
    if args["pos"]:
        args["pos"] = POS_CLI_TO_LEO.get(args["pos"], args["pos"])
    return args
